...
const predictGraph= function getServiceGraph(userid,callBack){
	...
	var cypher ='MATCH p = (: customer{ name: "moses" }) -[r: BOOKED] -> (s: service) -[h: BOOKED] - (c: customer) -[b: BOOKED] - (ss: service) -[pr:PROVIDES]-(sln:salon) WITH {service:ss{ss,salon:sln}} as ssv RETURN {services:collect(ssv)} LIMIT 5'
